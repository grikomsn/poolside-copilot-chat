...
Tests are colocated with the modules they cover. `npm test` performs a clean compile and runs credential-storage, model-filtering, stream-parser, and usage tests. `npm run package` validates the project and creates an installable VSIX.
